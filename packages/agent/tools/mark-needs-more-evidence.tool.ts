...
function assertAgentCanMarkNeedsMoreEvidence(input: {
  status: string;
}) {
...
export const markNeedsMoreEvidenceTool = tool(
    ...
    try {
      const result = await prisma.$transaction(async (tx) => {
        ...
        if (run.reviewTask) {
          ...
          assertAgentCanMarkNeedsMoreEvidence({
            status: run.reviewTask.status,
          });
          ...
          await tx.reviewEvent.create({
            data: {
              ...
              message:
                "Agent marked this review task as needing more evidence.",
          ...
          data: {
            ...
            events: {
              create: {
                type: "REVIEW_MORE_INFO_REQUESTED",
                message:
                  "Agent created a review task that needs more evidence.",
                metadata: toPrismaJson({
                  ...reasonJson,
                  previousStatus: null,
                  newStatus: "NEEDS_MORE_INFO",
  ...
  },
  {
    ...
    description:
      "Mark the review workflow as NEEDS_MORE_INFO when required claim evidence is missing. This is an agent routing action, not a human review decision. It does not send email and does not approve or deny the claim.",
