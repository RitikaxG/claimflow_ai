...
import { ClaimExtractionSchema } from "../packages/shared/schemas/claim_extraction";
import { ClaimValidationResultSchema } from "../packages/shared/schemas/claim_validation";
...
function compareField(
  ...
  return {
    ...
    expected,
    actual,
    passed: JSON.stringify(normalize(expected)) === JSON.stringify(normalize(actual)),
    scored: true,
  };
}
...
function compareValidation(
  ...
  const checks: ValidationCheck[] = [
    ...
    {
      name: "missingFields",
      expected: toSortedStringArray(expected.missingFields),
      actual: toSortedStringArray(actual.missingFields),
      passed: sameArray(
        toSortedStringArray(expected.missingFields),
        toSortedStringArray(actual.missingFields),
      ),
    },
    {
      name: "requiredEvidence",
      expected: toSortedStringArray(expected.requiredEvidence),
      actual: toSortedStringArray(actual.requiredEvidence),
      passed: sameArray(
        toSortedStringArray(expected.requiredEvidence),
        toSortedStringArray(actual.requiredEvidence),
      ),
    },
    {
      name: "conflicts.ruleId",
      expected: toSortedRuleIds(expected.conflicts),
      actual: toSortedRuleIds(actual.conflicts),
      passed: sameArray(
        toSortedRuleIds(expected.conflicts),
        toSortedRuleIds(actual.conflicts),
      ),
    },
    {
      name: "warnings.ruleId",
      expected: toSortedRuleIds(expected.warnings),
      actual: toSortedRuleIds(actual.warnings),
      passed: sameArray(
        toSortedRuleIds(expected.warnings),
        toSortedRuleIds(actual.warnings),
      ),
    },
  ];
...
async function evaluateSample(fileName: string): Promise<SampleResult> {
  ...
  if (expectedFinalStatus === "NEEDS_REVIEW" && actualFinalStatus === "COMPLETED") {
